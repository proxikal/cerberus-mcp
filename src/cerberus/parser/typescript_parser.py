"""
TypeScript parser using tree-sitter for accurate symbol extraction.
...
try:
    ...
    logger.warning("tree-sitter not available for TypeScript parsing, falling back to regex")


def parse_typescript_file(file_path: Path, content: str) -> List[CodeSymbol]:
    """
    Parse TypeScript file using tree-sitter for accurate symbol extraction.
    ...
    if TREE_SITTER_AVAILABLE:
        try:
            ...
            return _parse_typescript_file_regex(file_path, content, normalized_path)
    else:
        return _parse_typescript_file_regex(file_path, content, normalized_path)
    ...
    def visit_node(node, parent_class=None):
        ...
        # Type alias declarations
            ...
            if name_node:
                ...
                symbols.append(CodeSymbol(
                    name=type_name,
                    type="variable",  # Use "variable" type for type aliases
                    file_path=file_path,
                    start_line=node.start_point[0] + 1,
                    end_line=node.end_point[0] + 1,
                    signature=None,
        ...
        elif node.type == "method_signature" and parent_class:
            name_node = node.child_by_field_name("name")
            if name_node:
                method_name = name_node.text.decode('utf8')
...
def _extract_return_type(node) -> Optional[str]:
    """
    Extract return type annotation from a function/method node.

    Args:
        node: Tree-sitter node (function_declaration, method_definition, etc.)

    Returns:
        Return type string or None
    """
    # Find type_annotation node
    for child in node.children:
...
def _parse_typescript_file_regex(file_path: Path, content: str, normalized_path: str) -> List[CodeSymbol]:
    """
    ...
    for symbol_type, regex_pattern in queries.items():
        for match in regex_pattern.finditer(content):
            ...
            logger.debug(f"Found TS symbol (regex): {symbol_name} ({symbol_type}) on line {line_number}")
